...
serve(async (req) => {
  ...
  try {
        ...
        messages: [
          {
            ...
            החזר תשובה במבנה JSON הבא בדיוק - השתמש בשם המסלול שהמשתמש בקש:
            {
              "id": "similar-" + [id מקורי],
              "title": "[השם שהמשתמש בקש]",
          ...
          {
            role: 'user',
            content: `צור מסלול דומה בשם "${pathName}" על בסיס המסלול הבא. השתמש בשם "${pathName}" בדיוק כמו שהוא, ותוודא שהשאלות שומרות על הסדר המקורי במיוחד אם זה מסלול הבנת הנקרא:\n\n${JSON.stringify(categoryData, null, 2)}`
          }
        ],
        max_tokens: 4000,
        temperature: 0.7
    ...
    try {
      ...
      similarPath = JSON.parse(cleanContent);
    } catch (error) {
